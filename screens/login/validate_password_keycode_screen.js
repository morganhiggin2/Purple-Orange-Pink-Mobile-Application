...
export class ValidatePasswordKeyCodeScreen extends React.Component {
    constructor(props) {
        super(props);
    ...
    async validatePassword() {
        ...
        var result = await GlobalEndpoints.makeGetRequest(true, "/api/AccountManager/GetResetPasswordToken?keycode=" + this.state.keycode + "&username=" + GlobalProperties.user_name)
        .then((result) => {
            if (result == undefined) {
                successful = false;
            }
        ...
        if (successful) {
            ...
            if (result.request.status ==  200) {
                ...
                var password_reset_token = JSON.parse(result.request.response).password_reset_token;

                //go to next page with password reset token
                this.props.navigation.navigate("Reset Forgot Password Screen", {password_reset_token: password_reset_token});
                //go to next page
                    ...
                    <View style={info_styles.body}>
                        <View style={inline_attribute_styles.body}>
                            <Text style={inline_attribute_styles.title_text}>
                                To reset your password, first enter the key code from the validation email we just sent.
                            </Text>     
                        </View>
        ...
        return (
            <View style={[main_styles.page, {flex: 1}]}>
                <FlatList data={[{id: 1}]} keyExtractor={() => "dummy"} listEmptyComponent={null} renderItem={renderComponent} style={{zIndex: 99, flex: 1}}/>
                  <TouchableHighlight style={main_styles.button} underlayColor="#FFE1D6" onPress={() => {this.validatePassword();}}>
