...
export class EditActivityImagesScreen extends React.Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        if (this.props.route.params.activity_images) {
            for (var i = 0; i < this.props.route.params.activity_images.length; i++) {
                activity_images.push(<ProfileImage key={i} chooseImage={this.chooseImage} passFunction={this.handleChoosenProfilePicture} uri={this.props.route.params.activity_images[i]} id={i}/>);
            }
        }
        ...
        GlobalProperties.return_screen = "Edit Activity Images Screen";
        GlobalProperties.screen_props = {
            activity_images: this.props.route.params.activity_images,
        };
    }
    ...
    render() {
        ...
        if (this.state.activity_images) {
            max_images_height = (Math.floor(this.state.activity_images.length / 2) + 1) * Math.trunc(Dimensions.get('window').width * 0.40) + 15;
        }*/

        return (
            <View style={[main_styles.page, {flex: 1}]}>
                <ScrollView contentContainerStyle={{flexDirection: "row", flexWrap: "wrap", flexGrow: 1}}>
                    {this.state.activity_images.map((image) => (image))}
                    <EmptySpace key={0}/>
                </ScrollView>
            </View>
        );
    ...
    handleChoosenProfilePicture(uri, props) {
        ...
        this.state.activity_images[props.id] = uri;
